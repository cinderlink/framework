...
export type QueryInstruction<Row extends TableRow = TableRow, I extends keyof InstructionType<Row> = keyof InstructionType<Row>> = {
    instruction: I;
...
export interface WhereInstruction<Row extends TableRow = TableRow> {
    ...
    field: keyof Row;
    operation: Operation;
    value: Operation extends "in" | "!in" | "contains" | "!contains" ? Row[keyof Row][] : Operation extends "between" | "!between" ? [Row[keyof Row], Row[keyof Row]] : Row[keyof Row];
}
...
export interface QueryBuilderInterface<Row extends TableRow = TableRow> {
    ...
    where<Key extends keyof Row = keyof Row>(field: Key, operation: Operation, value: Row[Key][] | Row[Key]): QueryBuilderInterface<Row>;
    orderBy(field: keyof Row, direction: "asc" | "desc"): QueryBuilderInterface<Row>;
    ...
    update(values: Partial<Row> | ((row: Row) => Promise<Exclude<Row, "id">> | Exclude<Row, "id">)): QueryBuilderInterface<Row>;
...
export interface QueryResult<Row = TableRow> {
    ...
    pick<Keys extends (keyof Row)[]>(fields: Keys): QueryResult<Extract<Row, Keys>>;
    all(): Row[];
    count(): number;
}
export interface TableQueryInterface<Row extends TableRow = TableRow, Def extends TableDefinition<Row> = TableDefinition<Row>> extends QueryBuilderInterface<Row> {
